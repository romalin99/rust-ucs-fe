...
impl FieldIdUssMappingRepository {
    ...
    pub async fn find_one(&self, id: i64) -> Result<Option<FieldIdUssMapping>> {
        ...
        tokio::time::timeout(timeout, tokio::task::spawn_blocking(move || {
            let conn = pool.get().context("Oracle pool: get connection")?;
            let sql = format!("SELECT {} FROM TCG_UCS.FIELD_ID_USS_MAPPING WHERE ID = :1", COLUMNS);
            let rows = conn.query(&sql, &[&id]).context("find_one query")?;
            for row_result in rows {
                let row = row_result.context("read row")?;
    ...
    pub async fn find_one_for_update(&self, id: i64) -> Result<Option<(FieldIdUssMapping, r2d2::PooledConnection<super::OracleConnectionManager>)>> {
        let pool = self.pool.clone();
        let timeout = self.read_timeout;
    ...
    pub async fn find_by_field_id_and_mcs_id(&self, field_id: &str, mcs_id: i64) -> Result<Option<FieldIdUssMapping>> {
        let pool = self.pool.clone();
        let timeout = self.read_timeout;
        ...
        tokio::time::timeout(timeout, tokio::task::spawn_blocking(move || {
            ...
            let rows = conn.query(&sql, &[&fid, &mcs_id]).context("find_by_field_mcs query")?;
            for row_result in rows {
                let row = row_result.context("read row")?;
                return Ok(Some(Self::map_row(&row)?));
            }
    ...
    pub async fn find_by_field_id_and_uss_id(&self, field_id: &str, uss_id: i32) -> Result<Option<FieldIdUssMapping>> {
        let pool = self.pool.clone();
        let timeout = self.read_timeout;
        let fid = field_id.to_string();
        ...
        tokio::time::timeout(timeout, tokio::task::spawn_blocking(move || {
            ...
            let rows = conn.query(&sql, &[&fid, &uss_id]).context("find_by_field_uss query")?;
            for row_result in rows {
                let row = row_result.context("read row")?;
                return Ok(Some(Self::map_row(&row)?));
            }
    ...
    pub async fn find_list_by_field_id(&self, field_id: &str) -> Result<Vec<FieldIdUssMapping>> {
        ...
        tokio::time::timeout(timeout, tokio::task::spawn_blocking(move || {
            ...
            let rows = conn.query(&sql, &[&fid]).context("find_list_by_field_id query")?;
            let mut list = Vec::new();
            for row_result in rows {
                let row = row_result.context("read row")?;
                list.push(Self::map_row(&row)?);
    ...
    pub async fn update(&self, data: &FieldIdUssMapping) -> Result<u64> {
        ...
        tokio::time::timeout(timeout, tokio::task::spawn_blocking(move || {
            ...
            if affected == 0 {
                return Err(anyhow!("update field_id_uss_mapping: no rows affected for ID {}", id));
            }
            Ok(affected)
        }))
        .await
    ...
    pub async fn delete(&self, id: i64) -> Result<u64> {
        ...
        tokio::time::timeout(timeout, tokio::task::spawn_blocking(move || {
            ...
            if affected == 0 {
                return Err(anyhow!("delete field_id_uss_mapping: no rows affected for ID {}", id));
            }
            Ok(affected)
        }))
        .await
